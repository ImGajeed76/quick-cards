<script lang="ts">
	...
	const set = $derived<FlashcardSet | null>(
		payload?.kind === 'vocab' ? payload.set : null
	);
	...
	function exportTxt() {
		runDownload('txt', () =>
			saveFile(
				() => formatCards(set!, resolveTermSep(termSepValue), resolveCardSep(cardSepValue)),
				`${baseName}.txt`,
				'text/plain'
			)
		);
	...
	function exportCsv() {
		runDownload('csv', () => saveFile(() => toCsv(set!), `${baseName}.csv`, 'text/csv'));
	}
		...
		runDownload('pdf-list', () =>
			saveFile(
				async () => {
					...
					return generateListPDF(set!).output('arraybuffer');
				},
				`${baseName}-list.pdf`,
				'application/pdf'
			)
		...
		runDownload('pdf-cards', () =>
			saveFile(
				async () => {
					...
					return generateFlashcardsPDF(set!).output('arraybuffer');
				},
				`${baseName}-cards.pdf`,
				'application/pdf'
			)
		...
		runDownload('anki', async () => {
			const outcome = await saveFile(
				async () => {
					...
					return buildAnkiPackage({ set: set!, days: ankiDays, SQL });
				},
				`${baseName}.apkg`,
...
{:else if payload.kind === 'quizlet'}
	...
	<div class="mx-auto max-w-[640px] px-6 py-16 sm:py-20">
		...
		<h1 class="mt-12 text-3xl font-semibold tracking-tight">
			Quizlet links need the extension
		</h1>
		<p class="mt-3 text-[15px] leading-7 text-muted-foreground">
			We can't fetch Quizlet sets from the web directly — Cloudflare blocks cross-origin
			requests. The extension runs inside your own browser session, so it works around this
			without any workarounds of ours.
		</p>
	...
	<div class="mx-auto max-w-[640px] px-6 py-12">
		...
		<div class="mt-10 flex items-baseline justify-between gap-4">
			<input
				...
				class="min-w-0 flex-1 bg-transparent text-3xl font-semibold tracking-tight outline-none placeholder:text-muted-foreground/40"
			/>
			<span class="shrink-0 text-sm text-muted-foreground">
				<span class="tabular-nums text-foreground font-semibold">{cardCount}</span>
		...
		<div class="mt-8">
			...
			>
				<ChevronRight
					class="size-3.5 transition-transform {previewOpen ? 'rotate-90' : ''}"
				/>
				Preview
			</button>
			{#if previewOpen}
						...
						<tbody>
							{#each set.cards.slice(0, 20) as card, i (i)}
								<tr class="border-b border-border last:border-0 {i % 2 === 1 ? 'bg-muted/30' : ''}">
									<td class="px-3 py-1.5 font-medium max-w-0 truncate w-2/5">{card.term}</td>
									<td class="px-3 py-1.5 text-muted-foreground max-w-0 truncate">{card.definition}</td>
								</tr>
							{/each}
							{#if set.cards.length > 20}
								<tr>
									<td colspan="2" class="px-3 py-2 text-center text-xs text-muted-foreground">
		...
		<div class="mt-10 grid grid-cols-2 gap-4">
			<div class="space-y-1.5">
				<Label class="text-xs text-muted-foreground" for="term-sep">Term-Def separator</Label>
				<div class="relative">
					<Input id="term-sep" bind:value={termSepValue} placeholder="e.g. |" class="pr-9" />
					<Popover.Root>
						<Popover.Trigger
							class="absolute inset-y-0 right-0 inline-flex items-center px-2.5 text-muted-foreground hover:text-foreground"
			...
			<div class="space-y-1.5">
				<Label class="text-xs text-muted-foreground" for="card-sep">Card separator</Label>
				<div class="relative">
		...
		<div class="mt-8 flex items-center gap-3">
			...
			<span class="text-xs text-muted-foreground">Download as file</span>
			<Separator class="flex-1" />
		</div>
		...
		<Dialog.Root bind:open={ankiOpen}>
			<Dialog.Content class="sm:max-w-md">
				...
				<div class="space-y-5">
					<div class="space-y-1.5">
						<Label class="text-xs text-muted-foreground" for="anki-days">Days</Label>
						<Input
							id="anki-days"
							type="number"
							min="1"
					...
					<p class="text-center text-xs text-muted-foreground">{recommendedPace}</p>
				</div>
