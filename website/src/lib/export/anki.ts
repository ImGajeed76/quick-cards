...
export async function buildAnkiPackage(opts: AnkiExportOptions): Promise<Uint8Array> {
  ...
  //    media URL into the index along the way. Carry the per-card Quizlet ID
  //    alongside so we can build deterministic note GUIDs and Anki dedups
  //    on re-import.
  ...
  //    decides which templates render. When a card carries a Quizlet ID
  //    (extension flow, or share-link flow), we seed a stable GUID per
  //    notetype so re-importing the same set updates existing notes in
  //    place instead of creating duplicates. Cards without a Quizlet ID
  //    (most paste flows) fall through to ankipack's auto-generated GUID.
